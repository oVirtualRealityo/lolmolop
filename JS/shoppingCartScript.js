...
openSearch.addEventListener("click", function (e) {
    if (hiddenSearch.style.display === "none") {
...
buyButtons.forEach(function(button){
    button.addEventListener("click", function(e){
        ...
        addToCart(productName, productPrice, productImage); /* dit voegt het item toe aan het mandje indien nodig */
        totalCounts();
    });
});
...
function addToCart(name, price, image) {
    ...
    if (ItemInCartIndex !== -1) {
        ...
        cart.push({image: image, name: name, price: price , quantity: 1});
